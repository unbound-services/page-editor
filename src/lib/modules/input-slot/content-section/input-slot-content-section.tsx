...
export const ContentSection = (props: ContentSectionProps) => {
  ...
  if (editing && !renderFlags?.noAdd) {
    addButton = (<>
      ...
      <Drawer open={componentDrawerOpen} className="component-drawer" onClose={()=>setComponentDrawerOpen(false)}>
        <h1 className="component-drawer__h1">Insert Component</h1>
        <label>Search:<input placeholder="search" type="text" value={compSearch} onChange={(e)=>setCompSearch(e.target.value)} /></label>
        <div className="componeent-drawer__component-list">
            {sortedComponentList.map((componentName) => {
  ...
  if (isRoot && renderInIframe) {
    ...
    final = (
      <div
        ...
        style={{
          width:  viewportDimensions.width  * viewportDimensions.zoom/100.0,
          height: viewportDimensions.height * viewportDimensions.zoom/100.0
        }}
      >
        <iframe
        ...
        <div ref={handleRef} className={`page-editor__viewport-handle`}></div>
      </div>
    )
  }
...
const RenderComponents = ({
  ...
  return componentData.map((item, index) => {
    ...
    if (editing && context.componentList[item.comp]) {
      const componentDisplayName = context.componentList[item.comp].displayName
